...
const NewForm = (props) => {
    ...
    useEffect(() => {
        if (props.actionToTake === "update") {
            setTitle(clip.title)
            setLink(clip.link)
            setUploadDate(clip.datePublished.slice(0,clip.datePublished.indexOf("T")))
    ...
    return (
        <div className='form-wrapper'>
            ...
            <FormInput inputId={`${props.actionToTake}Title`} inputType="text" isRequired={true} pattern={".*"} valDescriptor="Episode Title:" beingChanged={title} changeFunction={setTitle} />

            <FormInput inputId={`${props.actionToTake}Link`} inputType="text" isRequired={true} pattern={".*"} valDescriptor="Link to episode:" beingChanged={link} changeFunction={setLink} />

            <FormInput inputId={`${props.actionToTake}Date`} inputType="text" isRequired={true} pattern={"(?:19\d{2}|20[01][0-9]|202[0-2])[-/.](?:0[1-9]|1[012])[-/.](?:0[1-9]|[12][0-9]|3[01])"} valDescriptor="Date uploaded:" beingChanged={uploadDate} changeFunction={setUploadDate} />

            <FormInput inputId={`${props.actionToTake}Start`} inputType="text" isRequired={false} pattern={"([0-9]{2}:)?[0-6][0-9]:[0-6][0-9]"} valDescriptor="Timestamp where the clip begins:" beingChanged={startTime} changeFunction={setStartTime} />

            <FormInput inputId={`${props.actionToTake}End`} inputType="text" isRequired={false} pattern={"([0-9]{2}:)?[0-6][0-9]:[0-6][0-9]"} valDescriptor="Timestamp where the clip ends:" beingChanged={endTime} changeFunction={setEndTime} />

            <FormInput inputId={`${props.actionToTake}Name`} inputType="text" isRequired={true} pattern=".*" valDescriptor="Podcast/Show name:" beingChanged={showName} changeFunction={setShowName} />
